...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    List<Booking> findLastBookingsForItems(@Param("itemIds") List<Long> itemIds, @Param("now") Instant now);
    ...
    List<Booking> findNextBookingsForItems(@Param("itemIds") List<Long> itemIds, @Param("now") Instant now);

    @Query("SELECT b.end FROM Booking b WHERE b.booker.id = :userId AND b.item.id = :itemId AND b.status = 'APPROVED'")
    Optional<Instant> findEndByBookerAndItem(@Param("userId") Long userId, @Param("itemId") Long itemId);
}
